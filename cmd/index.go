...
func SetupIndexer(cfg config.Config) *Indexer {
	var idxr Indexer
	var err error
	//TODO: split out setup methods and only call necessary ones
	idxr.cfg, idxr.db, idxr.scheduler, err = setup(cfg)
	if err != nil {
...
func Index(cfg config.Config) {
	...
	//Stubbed for now until we add Osmosis compatibility
